class PremixPdf < Prawn::Document
  ...
  def draw
    ...
    new_page
    repeat(:all) do
      draw_text "Optimize by - Least Cost Feed", size: 16, style: :bold, at: [5.mm, 287.mm]
      draw_text "Produce by - #{User.current.name} at #{@premix.updated_at.in_time_zone(User.current.time_zone).to_s(:long)}", size: 14, at: [5.mm, 278.mm]

      draw_text "Formula:", size: 12, at: [5.mm, 270.mm], style: :bold
      draw_text "#{@premix.name}", size: 12, at: [32.mm, 270.mm]
      
      draw_text "Batch Size:", size: 12, at: [100.mm, 270.mm], style: :bold
      draw_text "#{@premix.batch_size} #{User.current.weight_unit}", size: 12, at: [134.mm, 270.mm]
      
      draw_text "Ingredient", size: 12, style: :bold, at: [5.mm, 263.mm]
      draw_text "%", size: 12, style: :bold, at: [88.mm, 263.mm]
      draw_text "#{User.current.weight_unit}", size: 12, style: :bold, at: [108.mm, 263.mm]
      ...
      draw_text "#{@premix.name} Premix", size: 12, at: [32.mm, @py]
      
      draw_text "Batch Size:", size: 12, at: [100.mm, @py], style: :bold
      draw_text "#{@premix.total_premix_weight}  #{User.current.weight_unit}", size: 12, at: [134.mm, @py]

      @py = @py - @detail_height

      draw_text "Bag Weight:", size: 12, at: [5.mm, @py], style: :bold
      draw_text "#{@premix.premix_bag_weight} #{User.current.weight_unit}", size: 12, at: [32.mm, @py]
      
      draw_text "Total Bags:", size: 12, at: [100.mm, @py], style: :bold
      draw_text "#{@premix.bags_of_premix}", size: 12, at: [134.mm, @py]

      @py = @py - @detail_height - 2.mm

      draw_text "Ingredient", size: 12, style: :bold, at: [5.mm, @py]
      draw_text "%", size: 12, style: :bold, at: [88.mm, @py]
      draw_text "#{User.current.weight_unit}", size: 12, style: :bold, at: [108.mm, @py]
      draw_text "Bags", size: 12, style: :bold, at: [130.mm, @py]
      draw_text "Left (Kg)", size: 12, style: :bold, at: [155.mm, @py]
