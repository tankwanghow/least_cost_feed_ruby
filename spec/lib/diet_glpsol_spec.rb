...
describe DietGlpsol do
  before(:each) do
    @me      = double(id: 1, name: 'ME',      unit: 'kcal/kg')
    @protein = double(id: 2, name: 'Protein', unit: '%')
    @calcium = double(id: 3, name: 'Calcium', unit: '%')
    @nothing = double(id: 4, name: 'Nothing', unit: 'n')
    @formula_nutrients = [
      double(nutrient: @me,      max: nil, min: 2800), 
      double(nutrient: @protein, max:  16, min:  nil), 
      double(nutrient: @calcium, max: 4.1, min:  3.8),
      double(nutrient: @nothing, max: nil, min: nil)
    ]
    @formula1_nutrients = [
      double(nutrient: @me,      max: nil, min: 8000), 
      double(nutrient: @protein, max:  50, min:  nil), 
      double(nutrient: @calcium, max: 4.1, min:  3.8),
      double(nutrient: @nothing, max: nil, min: nil)
    ]
    @corn_nutrients = [
      double(nutrient: @me,      value: 3350), 
      double(nutrient: @protein, value:  8.3), 
      double(nutrient: @calcium, value: 0.02),
      double(nutrient: @nothing, value:    1)
    ]
    @soyabean_nutrients = [
      double(nutrient: @me,      value: 2450), 
      double(nutrient: @protein, value: 47.5), 
      double(nutrient: @calcium, value: 0.30),
      double(nutrient: @nothing, value:    2)
    ]
    @wheatbran_nutrients = [
      double(nutrient: @me,      value: 1300), 
      double(nutrient: @protein, value: 15.0), 
      double(nutrient: @calcium, value: 0.10),
      double(nutrient: @nothing, value:    4)
    ]
    @limestone_nutrients = [
      double(nutrient: @me,      value:    0), 
      double(nutrient: @protein, value:    0), 
      double(nutrient: @calcium, value: 38.0),
      double(nutrient: @nothing, value:    5)
    ]
    @mdcp_nutrients = [
      double(nutrient: @me,      value:    0), 
      double(nutrient: @protein, value:    0), 
      double(nutrient: @calcium, value: 18.0),
      double(nutrient: @nothing, value:    9)
    ]
    @corn      = double(id: 1, name: 'Corn',          cost: 0.9500, max: 0.70, min: 0.40, nutrients: @corn_nutrients)
    @soyabean  = double(id: 2, name: 'Soyabean Meal', cost: 1.8000, max:  nil, min: 0.15, nutrients: @soyabean_nutrients)
    @wheatbran = double(id: 3, name: 'Wheat Bran',    cost: 0.7900, max: 0.50, min: nil,  nutrients: @wheatbran_nutrients)
    @limestone = double(id: 4, name: 'Limestone',     cost: 0.0650, max:  nil, min: nil,  nutrients: @limestone_nutrients)
    @mdcp      = double(id: 5, name: 'MDCP',          cost: 1.7500, max:  nil, min: nil,  nutrients: @mdcp_nutrients)
    @ingredients = [@corn, @soyabean, @wheatbran, @limestone, @mdcp]
    @formula   = double(:ingredients => @ingredients, nutrients: @formula_nutrients)
    @formula1   = double(:ingredients => @ingredients, nutrients: @formula1_nutrients)
  end
  ...
  it "should generate printf for ingredients" do
    DietGlpsol.printf_statement_for_ingredients(@ingredients).should == 
  ...
  it "should generate nutrients expressions constraints" do
    DietGlpsol.nutrient_expressions_constraints(@formula_nutrients, @ingredients).should == 
  ...
  it "should generate gt zero constraint" do
    DietGlpsol.nutrient_expressions(@nothing, @ingredients).should == "+1*p_1 +2*p_2 +4*p_3 +5*p_4 +9*p_5"
  end

  it "should generate nutrient l.t. expressions constraint" do
    DietGlpsol.nutrient_expressions_constraint(@formula_nutrients[1], @ingredients).should == "n_2: +8.3*p_1 +47.5*p_2 +15.0*p_3 <= 16;"
  end

  it "should generate nutrient g.t. expressions constraint" do
    DietGlpsol.nutrient_expressions_constraint(@formula_nutrients[0], @ingredients).should == "n_1: +3350*p_1 +2450*p_2 +1300*p_3 >= 2800;"
  end

  it "should generate nutrient g.t. and l.t. expressions constraint" do
    DietGlpsol.nutrient_expressions_constraint(@formula_nutrients[2], @ingredients).should == "n_3: 3.8 <= +0.02*p_1 +0.3*p_2 +0.1*p_3 +38.0*p_4 +18.0*p_5 <= 4.1;"
  end

  it "should generate nutrient expressions" do
    DietGlpsol.nutrient_expressions(@protein, @ingredients).should == "+8.3*p_1 +47.5*p_2 +15.0*p_3"
  end
  ...
  it "should generate 100% default constraint" do
    DietGlpsol.constraint_100(@ingredients).should == "PERC: +p_1 +p_2 +p_3 +p_4 +p_5 = 1;\n"
  end

  it "should generate ingredients constraints" do
    DietGlpsol.ingredients_constraints(@ingredients).should ==
  ...
  it "should generate zero constraint" do
    DietGlpsol.ingredient_constraint(@limestone).should == "s.t.pc_4: p_4 >= 0;"
  end

  it "should generate l.t. ingredient constraint" do
    DietGlpsol.ingredient_constraint(@wheatbran).should == "s.t.pc_3: p_3 <= 0.5;"
  end

  it "should generate g.t. ingredient constraint" do
    DietGlpsol.ingredient_constraint(@soyabean).should == "s.t.pc_2: p_2 >= 0.15;"
  end

  it "should generate g.t. and l.t. ingredient constraint" do
    DietGlpsol.ingredient_constraint(@corn).should == "s.t.pc_1: 0.4 <= p_1 <= 0.7;"
  end

  it "should generate minimize objective function" do
    DietGlpsol.objective_function(@ingredients).should == "minimize cost: +0.95*p_1 +1.8*p_2 +0.79*p_3 +0.065*p_4 +1.75*p_5;\n"
  end
  ...
  it "should generate_varibles string for given ingredients" do
    DietGlpsol.varibles(@ingredients).should == "var p_1 >= 0;\nvar p_2 >= 0;\nvar p_3 >= 0;\nvar p_4 >= 0;\nvar p_5 >= 0;\n"
  end
end